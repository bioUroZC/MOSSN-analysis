...
base_dir <- "/proj/c.zihao/work1/1NT/2CPTAC/1distance"
...
method_levels <- c(
  "MOSSN_noPrior","MOSSN_uniform","MOSSN_noSeed","MOSSN_noRWR","MOSSN_noCorr",
...
# fill missing Patkar/KIRC rows
cluster_raw <- bind_rows(
  cluster_raw,
  data.frame(
    method           = "Patkar",
    feature_fraction = fraction_levels,
    cancer           = "KIRC",
    accuracy         = 0.5,
    auc              = 0.5,
    stringsAsFactors = FALSE
...
# extract shared legend from line plots (both use identical method colours)
...
ggsave(file.path(out_dir, "combined_4panel.pdf"), combined,
       width = 14, height = 9)
