...
cancers <- c("LUAD", "eLUAD", "KIRC", "PRAD")

metadata <- do.call(rbind, lapply(cancers, function(ds) {
    f <- file.path(base_dir, ds, paste0(ds, "_exprSet_filtered.csv"))
    if (!file.exists(f)) return(NULL)
    samples <- colnames(fread(f, nrows = 0))[-1]
    data.frame(Sample = samples, Type = ds, stringsAsFactors = FALSE)
}))
write.csv(metadata, file.path(output_dir, "metadata.csv"), row.names = FALSE)
cat("Metadata:", nrow(metadata), "samples across", length(cancers), "datasets\n")
...
for (i in seq_along(method_files)) {
  ...
  for (cancer in cancers) {
    ...
    for (top_frac in top_fracs) {
      ...
      sample_groups <- ifelse(grepl("_Normal$", sample_ids), "Normal", "Tumor")
