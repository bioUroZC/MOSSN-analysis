...
network <- "STRING"
matrix_dir   <- "/proj/c.zihao/work1/1NT/2string"
backbone_csv <- "/proj/c.zihao/work1/1NT/1data/string/links.csv"
methods      <- c(
  "MOSSN_noPrior", "MOSSN_uniform",
  "MOSSN_noSeed", "MOSSN_noRWR", "MOSSN_noCorr", 
  "SSN", "SWEET", "LIONESS", "Patkar", "PPIXpress", "Proteinarium"
)

n_cores <- max(1L, as.integer(Sys.getenv("SLURM_CPUS_PER_TASK", "1")))
out_dir <- file.path("/proj/c.zihao/work1/1NT/2free", network)
...
luad_samples <- setdiff(
  names(read_csv("/proj/c.zihao/work1/1NT/2string/SSN/LUAD/Zscore.csv",
...
fit_scalefree <- function(n1, n2, w, top_pct) {
  ...
  deg <- as.integer(table(c(n1[ord], n2[ord])))
  deg <- deg[deg > 0L]
...
read_matrix_long <- function(method) {
  f <- file.path(matrix_dir, method, "merged_matrix.csv")
