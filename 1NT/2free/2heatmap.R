...
network  <- "STRING"
data_dir <- file.path("/proj/c.zihao/work1/1NT/2free", network)
...
method_order <- c(
  "Backbone (STRING)",
  "MOSSN_noPrior", "MOSSN_uniform", "MOSSN_noSeed",
  "MOSSN_noRWR", "MOSSN_noCorr",
  "SSN", "SWEET", "LIONESS", "Patkar", "PPIXpress", "Proteinarium"
)

# ── backbone stats per threshold ──────────────────────────────────────────────
bb_links <- read_csv(
  "/proj/c.zihao/work1/1NT/1data/string/links.csv",
  show_col_types = FALSE
...
sum_gamma <- dat %>%
  ...
  mutate(
    ...
    label        = ifelse(mean_gamma > 4,
                          sprintf("%.2f*", mean_gamma),
                          sprintf("%.2f",  mean_gamma))
  )
