...
# 1. Basic settings
BASE_DIR <- paste0(PROJ_ROOT, "/1NT/4net/2biogrid")
LINKS_FILE <- paste0(PROJ_ROOT, "/1NT/1data/biogrid/biogrid_link.csv")
...
if (nrow(mossn_noPrior_long) > 0) {
    ...
    readr::write_csv(mossn_noPrior_matrix, file.path(BASE_DIR, method, "merged_matrix.csv"))
...
mossn_noSeed_long <- dplyr::bind_rows(all_df)
rm(all_df)
if (nrow(mossn_noSeed_long) > 0) {
    mossn_noSeed_matrix <- make_matrix_from_long(mossn_noSeed_long)
    rm(mossn_noSeed_long)
    readr::write_csv(mossn_noSeed_matrix, file.path(BASE_DIR, method, "merged_matrix.csv"))
    cat(method, "->", nrow(mossn_noSeed_matrix), "interactions x", ncol(mossn_noSeed_matrix) - 1, "samples\n")
    rm(mossn_noSeed_matrix)
} else {
    rm(mossn_noSeed_long)
    cat(method, ": no input files found\n")
}
...
mossn_noCorr_long <- dplyr::bind_rows(all_df)
rm(all_df)
if (nrow(mossn_noCorr_long) > 0) {
    mossn_noCorr_matrix <- make_matrix_from_long(mossn_noCorr_long)
    rm(mossn_noCorr_long)
    readr::write_csv(mossn_noCorr_matrix, file.path(BASE_DIR, method, "merged_matrix.csv"))
    cat(method, "->", nrow(mossn_noCorr_matrix), "interactions x", ncol(mossn_noCorr_matrix) - 1, "samples\n")
    rm(mossn_noCorr_matrix)
} else {
    rm(mossn_noCorr_long)
    cat(method, ": no input files found\n")
}
...
mossn_noRWR_long <- dplyr::bind_rows(all_df)
rm(all_df)
if (nrow(mossn_noRWR_long) > 0) {
    mossn_noRWR_matrix <- make_matrix_from_long(mossn_noRWR_long)
    rm(mossn_noRWR_long)
    readr::write_csv(mossn_noRWR_matrix, file.path(BASE_DIR, method, "merged_matrix.csv"))
    cat(method, "->", nrow(mossn_noRWR_matrix), "interactions x", ncol(mossn_noRWR_matrix) - 1, "samples\n")
    rm(mossn_noRWR_matrix)
} else {
    rm(mossn_noRWR_long)
    cat(method, ": no input files found\n")
}
