...
base_dir <- "/proj/c.zihao/work1/1NT/3robust/30"
...
selected_samples <- sort(
  sample(
    tumor_samples,
    size = max(1, ceiling(length(tumor_samples) * 0.3)),
    replace = FALSE
  )
)
...
lung_half_n <- max(1, ceiling(lung_n * 0.3))
...
cat("Selected 30% tumor samples:", half_num, "\n")
...
cat("Selected 30% Lung normal samples:", lung_half_n, "\n")
