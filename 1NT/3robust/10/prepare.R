...
base_dir <- "/proj/c.zihao/work1/1NT/3robust/10"
...
selected_samples <- sort(
  sample(
    tumor_samples,
    size = max(1, ceiling(length(tumor_samples) * 0.1)),
    replace = FALSE
  )
)
...
lung_half_n <- max(3, ceiling(lung_n * 0.1))
...
cat("Selected 10% tumor samples:", half_num, "\n")
...
cat("Selected 10% Lung normal samples:", lung_half_n, "\n")
