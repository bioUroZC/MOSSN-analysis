...
long_df <- lapply(files, function(f) {
  ...
  read.table(f, header = TRUE, fill = TRUE) %>%
    dplyr::transmute(protein1 = gene1, protein2 = gene2, Sample = sample_id, Weight = raw_edge_score)
}) %>% dplyr::bind_rows()
save_merged(method, long_df)
...
long_df <- lapply(files, function(f) {
  ...
  readr::read_tsv(f, show_col_types = FALSE) %>%
    dplyr::transmute(protein1 = gene1, protein2 = gene2, Sample = sample_id, Weight = score)
}) %>% dplyr::bind_rows()
save_merged(method, long_df)
