...
gamma_dir <- file.path(base_dir, "gamma")
out_dir <- file.path(gamma_dir, "auc")
...
g_dirs <- list.dirs(gamma_dir, recursive = FALSE, full.names = TRUE)
g_dirs <- g_dirs[basename(g_dirs) != "auc"]
g_dirs <- g_dirs[basename(g_dirs) != "distance"]
g_dirs <- g_dirs[basename(g_dirs) != "plots"]

for (g_dir in g_dirs) {
  g_name <- basename(g_dir)
  matrix_file <- file.path(g_dir, "merged_matrix.csv")
  ...
  summary_row$gamma <- g_name
  summary_rows[[g_name]] <- summary_row
...
summary_df <- bind_rows(summary_rows) |>
  select(
    gamma, n_sample, n_variable_features, n_selected_features,
    accuracy, precision, recall, f1, auc, aupr
  )

write.csv(summary_df, file.path(out_dir, "gamma_metrics_summary.csv"), row.names = FALSE)
