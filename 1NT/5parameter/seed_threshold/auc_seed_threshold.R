...
seed_dir <- file.path(base_dir, "seed_threshold")
out_dir <- file.path(seed_dir, "auc")
...
threshold_dirs <- list.dirs(seed_dir, recursive = FALSE, full.names = TRUE)
threshold_dirs <- threshold_dirs[basename(threshold_dirs) != "auc"]
threshold_dirs <- threshold_dirs[basename(threshold_dirs) != "distance"]
threshold_dirs <- threshold_dirs[basename(threshold_dirs) != "plots"]

for (threshold_dir in threshold_dirs) {
  threshold_name <- basename(threshold_dir)
  matrix_file <- file.path(threshold_dir, "merged_matrix.csv")
  ...
  summary_row$seed_quantile <- threshold_name
  summary_rows[[threshold_name]] <- summary_row
...
summary_df <- bind_rows(summary_rows) |>
  select(
    seed_quantile, n_sample, n_variable_features, n_selected_features,
    accuracy, precision, recall, f1, auc, aupr
  )

write.csv(summary_df, file.path(out_dir, "seed_threshold_metrics_summary.csv"), row.names = FALSE)
