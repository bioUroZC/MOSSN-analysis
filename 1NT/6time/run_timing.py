...
sys.path.insert(0, "/proj/c.zihao/work1/function")

BASE_DIR    = "/proj/c.zihao/work1/1NT/6time"
DATA_DIR    = os.path.join(BASE_DIR, "data")
RESULTS_DIR = os.path.join(BASE_DIR, "results")
NORMAL_REF  = "/proj/c.zihao/work1/0ref/Test8/combined_expr_df.csv"
ORGAN       = "Lung"
...
def run_MOSSN_noPrior():
    import MOSSN_noPrior  # resolved at runtime via sys.path
    def _run():
        links = pd.read_csv(LINKS_FILE,   index_col=0)
        expr  = pd.read_csv(EXPR_FILE,    index_col=0)
        G, weights, e = MOSSN_noPrior.prepare_data_MOSSN_noPrior(
            links=links, expression_data=expr)
        for sid in e.columns:
            MOSSN_noPrior.MOSSN_noPrior_single_sample(
...
METHODS = {
    ...
    "MOSSN_noPrior": run_MOSSN_noPrior,
}
