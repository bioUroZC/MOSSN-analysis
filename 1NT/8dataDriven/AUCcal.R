...
output_dir    <- file.path(base_dir, "1distance")
...
make_metric_summary <- function(df, metric_name, frac_label) {
    ...
    out <- reshape(sub_df, idvar = "cancer", timevar = "method", direction = "wide")
...
for (i in seq_along(method_files)) {
    if (method_names[i] %in% skip_methods) {
        cat("\n[Method]", method_names[i], ": skipped\n")
        next
    }
    ...
    if (length(id_col) == 0) stop("No interaction ID column found in: ", method_files[i])
    ...
    for (cancer in cancers) {
        ...
        for (top_frac in top_fracs) {
            ...
            pc1 <- pca_res$x[, 1]
            if (mean(pc1[label_pos == 1]) < mean(pc1[label_pos == 0])) pc1 <- -pc1
...
write.csv(cluster_df, file.path(output_dir, "results_Cluster.csv"), row.names = FALSE)
cat("Saved to", file.path(output_dir, "results_Cluster.csv"), "\n")

metric_summaries <- lapply(
    setNames(
        paste0(as.integer(top_fracs * 100), "%"),
        paste0("top_", as.integer(top_fracs * 100))
    ),
    function(frac_label) {
        list(
...
print(list(overall_metrics = overall_metrics))
