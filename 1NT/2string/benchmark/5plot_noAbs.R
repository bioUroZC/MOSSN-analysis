...
base_dir <- paste0(PROJ_ROOT, "/1NT/2string/benchmark/1result")
out_dir  <- base_dir
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

cluster_raw <- read.csv(file.path(base_dir, "results_Cluster_noAbs.csv"),
                        stringsAsFactors = FALSE)
cluster_raw$method[cluster_raw$method == "MOSSN_uniform"] <- "MOSSN"
...
ggsave(file.path(out_dir, "Bench_noAbs_heatmap_accuracy.pdf"), p_acc,
       width = 8.5, height = 4.2)
ggsave(file.path(out_dir, "Bench_noAbs_heatmap_auc.pdf"), p_auc,
       width = 8.5, height = 4.2)
...
ggsave(file.path(out_dir, "Bench_noAbs_line_auc.pdf"), p_line_auc,
       width = 6, height = 4)
ggsave(file.path(out_dir, "Bench_noAbs_line_accuracy.pdf"), p_line_acc,
       width = 6, height = 4)

write.csv(summary_df, file.path(out_dir, "cluster_summary_noAbs.csv"),
          row.names = FALSE)
