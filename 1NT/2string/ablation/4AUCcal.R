...
# RawExpr and NodeRWR operate on gene-level matrices with far fewer rows
# than the edge-level methods (~12k genes vs ~98k interactions), so
# selecting "top X%" gives them a much smaller absolute feature budget at
# the same nominal fraction. RandomBackbone is included here too so its
# selection always tracks MOSSN_noSeed explicitly rather than relying on
# its own pool size happening to match. To keep the comparison on equal
# footing, all three reuse the absolute feature count MOSSN_noSeed selects
# for the same cancer/fraction instead of computing their own percentage.
absolute_topn_ref <- "MOSSN_noSeed"
absolute_topn_methods <- c("RawExpr", "NodeRWR", "RandomBackbone")

absolute_topn <- list()
ref_idx <- which(method_names == absolute_topn_ref)
if (length(ref_idx) == 1) {
  ref_mat <- fread(method_files[ref_idx]) |> as.data.frame()
  ref_id_col <- intersect(c("Interaction", "V1", "Unnamed: 0"), colnames(ref_mat))
  rownames(ref_mat) <- ref_mat[[ref_id_col[1]]]
  ref_mat[[ref_id_col[1]]] <- NULL
  ref_mat <- abs(ref_mat)

  for (cancer in cancers) {
    cancer_samples <- metadata$Sample[metadata$Type == cancer]
    cols <- intersect(cancer_samples, colnames(ref_mat))
    if (length(cols) < 4) next

    sub_mat <- ref_mat[, cols, drop = FALSE]
    ...
    for (top_frac in top_fracs) {
      ...
      absolute_topn[[cancer]][[frac_label]] <- max(1, floor(length(cv) * top_frac))
    }
  }
  rm(ref_mat)
} else {
  warning(
    "Reference method '", absolute_topn_ref, "' not found (or ambiguous); ",
    "RawExpr/NodeRWR will fall back to their own percentage-based selection."
  )
}
...
for (i in seq_along(method_files)) {
  ...
  for (cancer in cancers) {
    ...
    for (top_frac in top_fracs) {
      ...
      ref_topn <- absolute_topn[[cancer]][[frac_label]]
      if (method_names[i] %in% absolute_topn_methods && !is.null(ref_topn)) {
        top_n <- ref_topn
      } else {
