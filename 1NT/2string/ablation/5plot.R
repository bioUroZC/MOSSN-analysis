...
ablation_file  <- paste0(PROJ_ROOT, "/1NT/2string/ablation/1result/results_Cluster.csv")
benchmark_file <- paste0(PROJ_ROOT, "/1NT/2string/benchmark/1result/results_Cluster.csv")
out_dir        <- paste0(PROJ_ROOT, "/1NT/2string/ablation/1result")
...
# MOSSN is taken from the benchmark results; the remaining methods come from ablation.
mossn <- benchmark_raw %>%
  filter(method == "MOSSN_uniform") %>%
  mutate(method = "MOSSN")
...
method_colors <- c(
  ...
  EdgeNoRWR      = "#009E73",
  MOSSN_noSeed   = "#E69F00",
  RandomBackbone = "#B15928",
  PermutedControl = "#636363",
  NodeRWR        = "#0072B2",
  RawExpr        = "#CC79A7"
)
...
ggsave(file.path(out_dir, "Aba_heatmap_accuracy.pdf"), p_acc,
       width = 8.5, height = 4.7)
ggsave(file.path(out_dir, "Aba_heatmap_auc.pdf"), p_auc,
       width = 8.5, height = 4.7)
...
ggsave(file.path(out_dir, "Aba_line_auc.pdf"), p_line_auc,
       width = 6, height = 4)
ggsave(file.path(out_dir, "Aba_line_accuracy.pdf"), p_line_acc,
       width = 6, height = 4)
