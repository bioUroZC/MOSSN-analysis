...
def prepare_data_MOSSN_uniform(links, expression_data, uniform_weight=1.0):
    ...
    for _, row in links.iterrows():
        G.add_edge(row['protein1'], row['protein2'], weight=uniform_weight)

    uniform_original_weights = {
        (row['protein1'], row['protein2']): float(uniform_weight)
        for _, row in links.iterrows()
    }

    return G, uniform_original_weights, expression_data


def MOSSN_uniform_single_sample(
    sample_id,
    G,
    uniform_original_weights,
    expression_data,
    ...
    for u, v, data in G.edges(data=True):
        ...
        base_weight = uniform_original_weights.get(
            (u, v), uniform_original_weights.get((v, u), 0)
        )
    ...
    for u, v, data in G.edges(data=True):
        ...
        edges_data.append({
            ...
            "Node1": u,
            "Node2": v,
            "BaseWeight": uniform_original_weights.get(
                (u, v), uniform_original_weights.get((v, u), 0)
            ),
