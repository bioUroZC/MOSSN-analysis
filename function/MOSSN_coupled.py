...
def prepare_data_MOSSN_Coupled(links, omic_data, uniform_weight=1.0):
    """
    Build shared PPI graph on gene space = intersection of all active omics + PPI.
    The reference PPI network is treated as an unweighted topology: all retained
    edges are initialized with the same weight (uniform_weight), not the STRING
    confidence score.
    ...
    for _, row in link_filtered.iterrows():
        ...
        G.add_edge(u, v, weight=uniform_weight)
        real_original_weights[(u, v)] = uniform_weight
...
def prepare_data_MOSSN_Direct(links, omic_data, direct_omics, uniform_weight=1.0):
    """
    Build a mixed graph:
      - EXP layer: full PPI network  (nodes = gene names), edges initialized with
        a uniform base weight rather than the STRING confidence score
...
def prepare_data_MOSSN_Multilayer(links, omic_data, active_omics, uniform_weight=1.0):
    """
    Build a full multilayer graph:
      - Each omic has its own PPI layer: 'OMIC:gene' nodes with intra-layer PPI
        edges, initialized with a uniform base weight rather than the STRING
        confidence score
    ...
    for omic in active_omics:
        for _, row in link_filtered.iterrows():
            ...
            G.add_edge(u, v, weight=uniform_weight, edge_type="intra", omic=omic)
            weights[(u, v)] = uniform_weight
...
def MOSSN_Direct_single_sample(sample_id, G, real_original_weights, omic_data,
    ...
    P_exp_sub = P[exp_idx]
    ranks_sub = rankdata(P_exp_sub, method="average")
    norm_sub  = (ranks_sub - 1) / max(len(ranks_sub) - 1, 1)
