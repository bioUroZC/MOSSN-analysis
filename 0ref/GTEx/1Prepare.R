...
setwd(paste0(PROJ_ROOT, "/0ref/GTEx/"))

GTEx=read.table('GTEx_Analysis_2017-06-05_v8_RNASeQCv1.1.9_gene_tpm.gct.gz',
                header = T,sep = '\t',skip = 2)
...
for (organ in organs) {
  ...
  sample_meta <- pheno %>% filter(SMTS == organ)
  ...
  organ_expr_t <- organ_expr_t %>%
    ...
    relocate(sample_id, organ)  # Move metadata columns to front
  
  sample_df_list[[organ]] <- organ_expr_t
}

# Combine all organs into one dataframe
combined_expr_df <- bind_rows(sample_df_list)
