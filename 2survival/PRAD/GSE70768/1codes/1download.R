...
setwd("/proj/c.zihao/work1/1survival/PRAD/GSE70768/data")
...
pd <- subset(pd, pd$`sample type:ch1` == "Tumour")

pd <- subset(pd, select=c("geo_accession",  
                          'age at diag:ch1', 'clinical stage:ch1', 
                          "biochemical relapse (bcr):ch1", 
                          "time to bcr (months):ch1", 
                          'total follow up (months):ch1'))

names(pd) <- c("Sample", "Age", "Stage", "OS",  "Time1", "Time2")
