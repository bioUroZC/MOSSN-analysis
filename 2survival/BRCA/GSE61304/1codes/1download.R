...
setwd("/proj/c.zihao/work1/1survival/BRCA/GSE61304/data")
...
exprSet <- as.data.frame(exprs(gse$GSE61304_series_matrix.txt.gz))
str(exprSet)
...
pd <- pData(gse$GSE61304_series_matrix.txt.gz)
head(pd)
names(pd)
...
str(pd)
table(pd$EFS_Time)
pd$EFS_Time[pd$EFS_Time=="NA"] <- NA
pd$EFS_Time <-  as.numeric(as.character(pd$EFS_Time))
pd$EFS_Time <- pd$EFS_Time / 12
