...
setwd("/proj/c.zihao/work1/1survival/BRCA/GSE42568/data")
...
exprSet <- as.data.frame(exprs(gse$GSE42568_series_matrix.txt.gz))
str(exprSet)
...
pd <- pData(gse$GSE42568_series_matrix.txt.gz)
head(pd)
names(pd)

table(pd$`tissue:ch1`)

pd <- subset(pd, pd$`tissue:ch1` == "breast cancer")
head(pd)
