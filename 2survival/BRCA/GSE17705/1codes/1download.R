...
setwd("/proj/c.zihao/work1/1survival/BRCA/GSE17705/data")
...
exprSet <- as.data.frame(exprs(gse$GSE17705_series_matrix.txt.gz))
str(exprSet)
...
pd <- pData(gse$GSE17705_series_matrix.txt.gz)
head(pd)
names(pd)

pd <- subset(pd, select=c("geo_accession", 
                          "distant relapse (1=dr, 0 censored):ch1",
                          "event time (years):ch1"))
