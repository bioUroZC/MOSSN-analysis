...
setwd("/proj/c.zihao/work1/1survival/BRCA/GSE25065/data")
...
exprSet <- as.data.frame(exprs(gse$GSE25065_series_matrix.txt.gz))
str(exprSet)
...
pd <- pData(gse$GSE25065_series_matrix.txt.gz)
head(pd)
names(pd)

pd <- subset(pd, select=c("geo_accession", 
                          "age_years:ch1",
                          "drfs_1_event_0_censored:ch1",
                          "drfs_even_time_years:ch1"))
