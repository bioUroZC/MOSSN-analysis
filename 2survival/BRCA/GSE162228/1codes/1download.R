...
setwd("/proj/c.zihao/work1/1survival/BRCA/GSE162228/data")
...
exprSet <- as.data.frame(exprs(gse$GSE162228_series_matrix.txt.gz))
str(exprSet)
...
pd <- pData(gse$GSE162228_series_matrix.txt.gz)
head(pd)
names(pd)
...
pd <- subset(pd, select=c("geo_accession", 
                          ...
                          "relapse free survival (years):ch1"))

head(pd)
str(pd)

colnames(pd) <- c("geo_accession", "Age", "os_event", "rfs_event", "t_os", "t_rfs")


table(pd$os_event)
pd$os_event[pd$os_event=="Death"] <- 1
pd$os_event[pd$os_event=="Alive"] <- 0
pd$os_event <- as.numeric(pd$os_event)
table(pd$os_event)

table(pd$rfs_event)
pd$rfs_event[pd$rfs_event=="relapse"] <- 1
pd$rfs_event[pd$rfs_event=="non-relapse"] <- 0
