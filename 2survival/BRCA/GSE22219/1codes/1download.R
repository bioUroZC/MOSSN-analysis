...
setwd("/proj/c.zihao/work1/1survival/BRCA/GSE22219/data")
...
exprSet <- as.data.frame(exprs(gse$GSE22219_series_matrix.txt.gz))
str(exprSet)
...
pd <- pData(gse$GSE22219_series_matrix.txt.gz)
head(pd)
names(pd)

pd <- subset(pd, select=c("geo_accession", 
                          "patient age:ch1",
                          "distant-relapse event:ch1",
                          "distant-relapse free survival:ch1"))

head(pd)
str(pd)

colnames(pd) <- c("Sample", "Age", "EFS", "EFS_Time")
