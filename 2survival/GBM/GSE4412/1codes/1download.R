...
setwd("/proj/c.zihao/work1/1survival/GBM/GSE4412/data/")
...
exprSet <- as.data.frame(exprs(gse$`GSE4412-GPL96_series_matrix.txt.gz`))
str(exprSet)
...
pd <- pData(gse$`GSE4412-GPL96_series_matrix.txt.gz`)
head(pd)
names(pd)

table(pd$`histology:ch1`)
pd <- subset(pd, pd$`histology:ch1` == "GBM")
