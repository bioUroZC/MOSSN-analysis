...
setwd("/proj/c.zihao/work1/1survival/STAD/GSE26901/data")
...
exprSet <- as.data.frame(exprs(gse$GSE26901_series_matrix.txt.gz))
str(exprSet)
...
pd <- pData(gse[["GSE26901_series_matrix.txt.gz"]])
head(pd)
names(pd)
...
names(pd)[1] <- "Patient"
pd$Patient[1:5]
pd$Patient <- gsub("_T", '', pd$Patient)
pd$Patient <- gsub("_", '', pd$Patient)





clinical <- read.xlsx("41467_2018_4179_MOESM5_ESM.xlsx", sheet = "KUCM")
names(clinical)
