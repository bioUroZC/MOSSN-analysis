...
setwd("/proj/c.zihao/work1/1survival/BLCA/GSE48276/data/")

#=======================================================

#=======================================================

gse<- getGEO("GSE48276", destdir = ".") 
...
exprSet <- as.data.frame(exprs(gse$GSE48276_series_matrix.txt.gz)) 
...
pd <- pData(gse$GSE48276_series_matrix.txt.gz)
head(pd)

pd <- subset(pd, select=c( "geo_accession" , "age (at diagnosis):ch1",
                           "gender:ch1"   ,   "cstage:ch1",
                           'os censor:ch1',"survival (mo):ch1"))
...
table(pd$OS)
pd$OS <- ifelse(pd$OS == 'uncensored', 0, 1)
