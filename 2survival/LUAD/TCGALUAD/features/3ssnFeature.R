...
projetcNa <- "TCGALUAD"
filenameR <- paste0("/proj/c.zihao/work1/1survival/LUAD/", projetcNa, "/SSN")
filenameR
setwd(filenameR)

df <- read.csv("delta.csv", header = T)
df[1:5,1:5]


df$Node1_clean <- pmin(df$Gene1, df$Gene2)
df$Node2_clean <- pmax(df$Gene1, df$Gene2)
...
write.csv(wrwData, file = 'ssnData.csv')
cat("files saved")
