...
setwd("/proj/c.zihao/work1/1survival/LUAD/GSE72094/data/")
...
colnames(gpl)
gpl <- gpl %>% dplyr::select(ID, "GeneSymbol")
...
gpl <- gpl %>%
  tidyr::separate(GeneSymbol, into = c('gene', 'symbol'), sep = " ", fill = "right")%>%
...
exprSet <- as.data.frame(exprs(gse[["GSE72094_series_matrix.txt.gz"]]))
str(exprSet)
...
pd <- pData(gse$GSE72094_series_matrix.txt.gz)
head(pd)


pd <- subset(pd, select=c("geo_accession", "gender:ch1", "age_at_diagnosis:ch1", 
                          "Stage:ch1",
                          "vital_status:ch1", "survival_time_in_days:ch1"))
