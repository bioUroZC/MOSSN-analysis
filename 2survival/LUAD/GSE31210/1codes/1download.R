...
setwd("/proj/c.zihao/work1/1survival/LUAD/GSE31210/data/")
...
exprSet <- as.data.frame(exprs(gse[["GSE31210_series_matrix.txt.gz"]]))
str(exprSet)
...
pd <- pData(gse$GSE31210_series_matrix.txt.gz)
table(pd$characteristics_ch1)
pd <- subset(pd, pd$characteristics_ch1  == "tissue: primary lung tumor")

pd <- subset(pd, select=c("geo_accession", "characteristics_ch1.2", "age (years):ch1", 
                          "pathological stage:ch1",
                          "death:ch1", "days before death/censor:ch1"))
...
table(pd$Gender)
pd$Gender[pd$Gender=="gender: female"] <- "Female"
pd$Gender[pd$Gender=="gender: male"] <- "Male"
table(pd$Gender)
