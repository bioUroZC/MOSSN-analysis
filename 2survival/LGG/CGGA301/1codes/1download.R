...
setwd("/proj/c.zihao/work1/1survival/LGG/CGGA301/data")

#=======================================================

#=======================================================

exprSet <- fread("CGGA.mRNA_array_301_gene_level.20200506.txt")
...
pd <- fread("CGGA.mRNA_array_301_clinical.20200506.txt")
head(pd)
names(pd)
table(pd$Histology)
table(pd$PRS_type)
...
LGG_grades <- c("WHO II", "WHO III")
LGG_Histology <- c("A", "AA", "O", "OA", "AO", "AOA")

pd <- pd[
  pd$Grade %in% LGG_grades & pd$Histology %in% LGG_Histology,
]
