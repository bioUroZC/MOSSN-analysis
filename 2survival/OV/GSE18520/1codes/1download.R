...
setwd("/proj/c.zihao/work1/1survival/OV/GSE18520/data/")
set.seed(1234)
...
exprSet <- as.data.frame(exprs(gse$GSE18520_series_matrix.txt.gz))
str(exprSet)
...
pd <- pData(gse$GSE18520_series_matrix.txt.gz)
head(pd)
names(pd)

table(pd$'tissue:ch1')
pd <- subset(pd, pd$`tissue:ch1` == 'papillary serous ovarian adenocarcinoma')
...
head(pd)
colnames(pd) <- c("Sample",  "Stage",  "survival")
head(pd)

str(pd)

pd$OS_Time <- as.numeric(gsub(" \\(A\\)", "", pd$survival))

pd$OS <- ifelse(grepl("\\(A\\)", pd$survival), 0, 1)
head(pd)
