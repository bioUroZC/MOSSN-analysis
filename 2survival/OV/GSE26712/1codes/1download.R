...
setwd("/proj/c.zihao/work1/1survival/OV/GSE26712/data/")
set.seed(1234)
...
exprSet <- as.data.frame(exprs(gse$GSE26712_series_matrix.txt.gz))
str(exprSet)
...
pd <- pData(gse$GSE26712_series_matrix.txt.gz)
head(pd)
names(pd)

table(pd$'tissue:ch1')

pd <- subset(pd, pd$`tissue:ch1` == 'Late-stage high-grade ovarian cancer')
...
table(pd$OS)
pd$OS[pd$OS=="DOD (dead of disease)"] <- 1
pd$OS[pd$OS=="AWD (alive with disease)"] <- 0
pd$OS[pd$OS=="NED (no evidence of disease)"] <- 0
