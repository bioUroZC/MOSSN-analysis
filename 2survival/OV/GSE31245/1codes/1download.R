...
setwd("/proj/c.zihao/work1/1survival/OV/GSE31245/data/")
set.seed(1234)
...
exprSet <- as.data.frame(exprs(gse$GSE31245_series_matrix.txt.gz))
str(exprSet)
...
pd <- pData(gse$GSE31245_series_matrix.txt.gz)
head(pd)
names(pd)
...
pd <- subset(pd, select=c( "geo_accession", 
                           "overall survival (censored 1=event)):ch1"  ,
