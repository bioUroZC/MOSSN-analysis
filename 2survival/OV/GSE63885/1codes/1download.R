...
setwd("/proj/c.zihao/work1/1survival/OV/GSE63885/data/")
set.seed(1234)
...
exprSet <- as.data.frame(exprs(gse$GSE63885_series_matrix.txt.gz))
str(exprSet)
...
pd <- pData(gse$GSE63885_series_matrix.txt.gz)
head(pd)
names(pd)
...
pd <- subset(pd, select=c( "geo_accession", 
                           "figo_stage:ch1" ,
                           "clinical status at last follow-up (awd - alive with disease; dod - dead from disease; ned - no evidence of disease):ch1"  ,
                           "os - overall survival [days]:ch1" 
))
