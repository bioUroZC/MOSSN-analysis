...
setwd("/proj/c.zihao/work1/1survival/ACC/GSE19750/data/")
...
exprSet <- as.data.frame(exprs(gse$GSE19750_series_matrix.txt.gz))
str(exprSet)
...
pd <- pData(gse$GSE19750_series_matrix.txt.gz)
head(pd)
table(pd$"source_name_ch1")

pd <- subset(pd, pd$"source_name_ch1"  == "tumor, flash-frozen")

head(pd)
names(pd)

pd <- subset(pd, select=c("geo_accession", "gender:ch1" ,  "age in years:ch1",
                          "Stage:ch1"  , 
                          "survival status:ch1",
                          "survival in years:ch1" ))
...
pd$Gender[pd$Gender=="NA"] <- NA
table(pd$Gender)

table(pd$Age)
pd$Age[pd$Age=='NA'] <- NA
pd$Age[pd$Age=='Unknown'] <- NA
pd$Age <- as.numeric(as.character(pd$Age))
table(pd$Age)

table(pd$Stage)
pd$Stage[pd$Stage=="NA"] <- NA
pd$Stage[pd$Stage=="Recurrence"] <- NA
pd$Stage[pd$Stage=="Unknown"] <- NA
table(pd$Stage)
