...
setwd("/proj/c.zihao/work1/1survival/ACC/GSE76019/data/")
...
gpl$gene <- gdata::trim(gpl$'Gene.Symbol')
gpl$'Gene.Symbol' <- NULL

#=======================================================

#=======================================================

exprSet <- as.data.frame(exprs(gse$GSE76019_series_matrix.txt.gz))
str(exprSet)
...
pd <- pData(gse$GSE76019_series_matrix.txt.gz)
head(pd)
names(pd)

table(pd$`efs.event:ch1`)
...
head(pd)
names(pd) <- c("Sample", "Stage", "OS", "OS_Time")
head(pd)
