...
rm(list=ls())
setwd("/proj/c.zihao/work1/1survival/CRC/GSE17537/data/")
library(dplyr)
library(tibble)
library(tidyr)
library(GEOquery)
...
exprSet <- as.data.frame(exprs(gse$GSE17537_series_matrix.txt.gz)) 
...
pd <- pData(gse$GSE17537_series_matrix.txt.gz)
head(pd)
...
table(pd$OS)
pd$OS <- ifelse(pd$OS == 'no death', 0, 1)
