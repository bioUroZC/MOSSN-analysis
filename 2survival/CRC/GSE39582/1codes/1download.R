...
rm(list=ls())
setwd("/proj/c.zihao/work1/1survival/CRC/GSE39582/data/")
library(dplyr)
library(tibble)
library(tidyr)
library(GEOquery)
...
exprSet <- as.data.frame(exprs(gse$GSE39582_series_matrix.txt.gz)) 
...
pd <- pData(gse$GSE39582_series_matrix.txt.gz)
head(pd)

pd <- subset(pd, select=c( "geo_accession" ,
                           "age.at.diagnosis (year):ch1" ,  "Sex:ch1",  
                           "tnm.stage:ch1",
                           "tnm.t:ch1", "tnm.n:ch1", "tnm.m:ch1",
                           "os.event:ch1"  ,  "os.delay (months):ch1"  ))
