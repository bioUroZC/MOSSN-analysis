...
setwd('/proj/c.zihao/work1/1survival/CRC/TCGACRC/data')
...
write.csv(exprSet, file = "READ.csv")
...
write.csv(exprSet, file = "COAD.csv")
cat("All projects processed successfully!\n")
...
COAD <- read.csv("COAD.csv", header = T, row.names = 1)

READ <- read.csv("READ.csv", header = T, row.names = 1)
...
exprSet <- merge(COAD, READ, by="genes")
...
survivaldata <- subset(survivaldata, select = c("bcr_patient_barcode", "type",
                                                "OS", "OS.time"))
names(survivaldata) <- c("Sample", "cancertype", "OS", "OS.time")

survivaldata <- na.omit(survivaldata)

cancername <- c("COAD", "READ")

survivaldata <- survivaldata[which(survivaldata$cancertype %in% cancername),]
head(survivaldata)
