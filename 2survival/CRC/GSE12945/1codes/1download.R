...
rm(list=ls())
setwd("/proj/c.zihao/work1/1survival/CRC/GSE12945/data/")
library(dplyr)
library(tibble)
library(tidyr)
library(GEOquery)
...
exprSet <- as.data.frame(exprs(gse$GSE12945_series_matrix.txt.gz)) 
...
pd <- pData(gse$GSE12945_series_matrix.txt.gz)
head(pd)

table(pd$`RectumOrColon:ch1`)

pd <- subset(pd, select=c( "geo_accession" , "RectumOrColon:ch1",
                           "AgeAtDiagnosis:ch1",  "Gender:ch1",   
                           'pT:ch1', 'pN:ch1', 'pM:ch1', 
                         "Death:ch1" ,  "OverallSurvival_months:ch1"  ))



names(pd) <- c("Sample", "Group",  "Age", "Gender", 
               "Tstage", "Nstage", "Mstage", "OS", "OS_Time")
table(pd$OS)
str(pd)


pd <- subset(pd, pd$Group == "Colon")
