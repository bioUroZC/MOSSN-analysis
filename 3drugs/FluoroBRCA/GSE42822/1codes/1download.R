...
setwd("/proj/c.zihao/work1/3drugs/FluoroBRCA/GSE42822/data")
...
exprSet <- as.data.frame(exprs(gse$GSE42822_series_matrix.txt.gz))
str(exprSet)
...
pd <- pData(gse$GSE42822_series_matrix.txt.gz)
names(pd)
head(pd)
...
pd <- subset(pd, select=c("geo_accession" , 'pcr (1) vs rd (0):ch1'))
