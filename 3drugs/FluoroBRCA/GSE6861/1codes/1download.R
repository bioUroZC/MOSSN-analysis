...
setwd("/proj/c.zihao/work1/3drugs/FluoroBRCA/GSE6861/data")
...
exprSet <- as.data.frame(exprs(gse$GSE6861_series_matrix.txt.gz))
str(exprSet)
...
pd <- pData(gse$GSE6861_series_matrix.txt.gz)
names(pd)
head(pd)
...
table(pd$Response)
pd$Response[pd$Response=='npCR - no pathological complete response'] <- 2
pd$Response[pd$Response=='pCR - pathological complete response'] <- 3
