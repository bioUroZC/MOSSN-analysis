...
dieasename <- "FluoroBRCA"
base_path <- "/proj/c.zihao/work1/3drugs/"

saveDir <- paste0(base_path, dieasename, "/2response/")
...
datasets <- c("GSE6861", "GSE20194", "GSE20271", "GSE22093", "GSE23988",
              "GSE32646", "GSE42822", "GSE50948", "GSE66305", "GSE140494")

combined_df <- data.frame()  

for (name in datasets) {
  file_path <- paste0(base_path, dieasename, "/", name, "/data/pd.csv")
  df <- read.csv(file_path, row.names = 1)
  
  if (all(c("Sample", "Response") %in% colnames(df))) {
    message("Include: ", name)
    
    df2 <- df[, c("Sample", "Response"), drop = FALSE]
...
table(combined_df$Response)

survdata <- combined_df
table(survdata$Response)
...
matrixfile <-  paste0("/proj/c.zihao/work1/3drugs/", dieasename, "/1Matrix")
