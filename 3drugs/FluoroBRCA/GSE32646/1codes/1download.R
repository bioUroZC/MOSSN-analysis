...
setwd("/proj/c.zihao/work1/3drugs/FluoroBRCA/GSE32646/data")
...
exprSet <- as.data.frame(exprs(gse$GSE32646_series_matrix.txt.gz))
str(exprSet)
...
pd <- pData(gse$GSE32646_series_matrix.txt.gz)
names(pd)
head(pd)

table(pd$`treatment.type:ch1`)
table(pd$`pcr (1) vs rd (0):ch1`)


pd <- subset(pd, select=c("geo_accession" , 
                          'age:ch1', 'clinical stage:ch1',
                          'pathologic response pcr ncr:ch1'))
names(pd) <- c("Sample", "Age", "Stage",  "Response")
