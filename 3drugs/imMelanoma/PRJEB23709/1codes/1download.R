...
express <- read.delim("ICB.Gide2019_Pembrolizumab-Nivolumab_Melanoma.self_subtract",
                  header=TRUE)
...
pd <- read.delim('ICB.Gide2019_Pembrolizumab-Nivolumab_Melanoma.clinical', header = T)
head(pd)
table(pd$Response)
names(pd)[1] <- "Sample"
...
pd$Sample <- paste0('PN', pd$Sample)
colnames(exprSet) <- paste0('PN', colnames(exprSet))
...
exprSet[1:5,1:5]
write.csv(exprSet, file = "exprSet1.csv")
write.csv(pd, file = "pd1.csv")
