...
BASE_DIR  <- "/proj/c.zihao/work1/3drugs/immune"
OUT_DIR   <- file.path(BASE_DIR, "1Matrix")
...
if (nrow(ssn_long) > 0) {
    ...
    rm(ssn_long)
    readr::write_csv(ssn_matrix, file.path(OUT_DIR, "SSN_Matrix.csv"))
...
if (nrow(sweet_long) > 0) {
    ...
    rm(sweet_long)
    readr::write_csv(sweet_matrix, file.path(OUT_DIR, "SWEET_Matrix.csv"))
...
if (nrow(lioness_long) > 0) {
    ...
    rm(lioness_long)
    readr::write_csv(lioness_matrix, file.path(OUT_DIR, "LIONESS_Matrix.csv"))
...
if (nrow(mossn_noPrior_long) > 0) {
    ...
    readr::write_csv(mossn_noPrior_matrix, file.path(OUT_DIR, "MOSSN_noPrior_Matrix.csv"))
...
if (nrow(mossn_uniform_long) > 0) {
    ...
    readr::write_csv(mossn_uniform_matrix, file.path(OUT_DIR, "MOSSN_uniform_Matrix.csv"))
...
if (nrow(patkar_long) > 0) {
    ...
    rm(patkar_long)
    readr::write_csv(patkar_matrix, file.path(OUT_DIR, "Patkar_Matrix.csv"))
...
if (nrow(ppixpress_long) > 0) {
    ...
    rm(ppixpress_long)
    readr::write_csv(ppixpress_matrix, file.path(OUT_DIR, "PPIXpress_Matrix.csv"))
...
if (nrow(proteinarium_long) > 0) {
    ...
    readr::write_csv(proteinarium_matrix, file.path(OUT_DIR, "Proteinarium_Matrix.csv"))
