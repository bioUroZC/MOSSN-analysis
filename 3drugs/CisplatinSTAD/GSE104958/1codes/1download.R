...
setwd("/proj/c.zihao/work1/3drugs/CisplatinSTAD/GSE104958/data/")
set.seed(1234)
...
exprSet <- as.data.frame(exprs(gse$GSE104958_series_matrix.txt.gz))
str(exprSet)
...
pd <- pData(gse$GSE104958_series_matrix.txt.gz)
head(pd)
names(pd)
...
table(pd$Response)
pd$Response[pd$Response=="pCR"] <- 3
pd$Response[pd$Response=="non-pCR"] <- 2
table(pd$Response)
